...
const HandleSubmit = async (event) => {
  ...
  if (usuario.nombre.length && usuario.apellido.length && usuario.contrasenia.length > 5 && usuario.email.length) {
      try {
  ...
  <div class="mb-2" >
    ...
    <input type="email" class="form-control" placeholder='Ingresa tu Correo' id="exampleInputEmail1" aria-describedby="emailHelp" onChange={(event)=>setusuario({...usuario,email:event.target.value})}/>
    
  </div>
  <div class="mb-3">
    <label for="InputPassword2" class="text-inputs-form" >Crea una contraseña</label>
    <input type="password" class="form-control" placeholder='Ingresa tu Contraseña' id="exampleInputPassword1" onChange={(event)=>setusuario({...usuario,contrasenia:event.target.value})}/>
  </div>
